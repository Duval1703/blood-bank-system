...
<body class="min-h-screen blood-pattern">
    ...
    <nav class="blood-gradient shadow-lg">
            ...
            <div class="flex items-center justify-between h-16">
                ...
                <div class="flex items-center space-x-3">
                    ...
                    <div class="hidden sm:block">
                        <h1 class="text-white text-lg sm:text-xl font-bold">Blood Bank</h1>
                        <p class="text-red-100 text-xs">Management System</p>
                    </div>
                    <div class="sm:hidden">
                        <h1 class="text-white text-base font-bold">BBMS</h1>
                    </div>
                </div>

                <!-- Mobile Menu Button -->
                <button id="mobile-menu-button" class="lg:hidden text-white hover:bg-red-700 p-2 rounded-md focus:outline-none focus:ring-2 focus:ring-white">
        ...
        <div id="mobile-menu" class="hidden lg:hidden border-t border-red-400">
            <div class="px-2 pt-2 pb-3 space-y-1">
                <a href="{{ route('blood-bank.dashboard') }}" 
                   class="text-white hover:bg-red-700 block px-3 py-2 rounded-md text-base font-medium {{ request()->routeIs('blood-bank.dashboard') ? 'bg-red-800' : '' }}">
                ...
                <a href="{{ route('blood-bank.inventory') }}" 
                   class="text-white hover:bg-red-700 block px-3 py-2 rounded-md text-base font-medium {{ request()->routeIs('blood-bank.inventory') ? 'bg-red-800' : '' }}">
                    <span class="flex items-center space-x-2">
                ...
                <a href="{{ route('blood-bank.donors') }}" 
                   class="text-white hover:bg-red-700 block px-3 py-2 rounded-md text-base font-medium {{ request()->routeIs('blood-bank.donors') ? 'bg-red-800' : '' }}">
                    <span class="flex items-center space-x-2">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"></path></svg>
                        <span>Donors</span>
                    </span>
                ...
                <a href="{{ route('blood-bank.distributions') }}" 
                   class="text-white hover:bg-red-700 block px-3 py-2 rounded-md text-base font-medium {{ request()->routeIs('blood-bank.distributions') ? 'bg-red-800' : '' }}">
                    <span class="flex items-center space-x-2">
                ...
                <a href="{{ route('blood-bank.partners') }}" 
                   class="text-white hover:bg-red-700 block px-3 py-2 rounded-md text-base font-medium {{ request()->routeIs('blood-bank.partners') ? 'bg-red-800' : '' }}">
                    <span class="flex items-center space-x-2">
                ...
                <a href="{{ route('blood-bank.alerts') }}" 
                   class="text-white hover:bg-red-700 block px-3 py-2 rounded-md text-base font-medium {{ request()->routeIs('blood-bank.alerts') ? 'bg-red-800' : '' }}">
                    <span class="flex items-center space-x-2">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9"></path></svg>
                        <span>Alerts</span>
                    </span>
                ...
                @if(auth()->check())
                    <div class="border-t border-red-400 mt-2 pt-2">
                        <div class="px-3 py-2">
                            <p class="text-white text-sm font-medium">{{ auth()->user()->name }}</p>
                            <p class="text-red-100 text-xs">{{ auth()->user()->role }}</p>
                        </div>
                        <form method="POST" action="{{ route('logout') }}" class="px-3 py-2">
                            @csrf
                            <button type="submit" class="w-full bg-white text-red-600 hover:bg-red-50 px-4 py-2 rounded-md text-sm font-medium transition-colors text-left">
                                Logout
                            </button>
                        </form>
                    </div>
                @else
                    <a href="{{ route('login') }}" 
                       class="block px-3 py-2">
                        <span class="block w-full bg-white text-red-600 hover:bg-red-50 px-4 py-2 rounded-md text-sm font-medium transition-colors text-center">
                            Login
                        </span>
                    </a>
                @endif
